...
function updateCursorPosition() {
  ...
  if (nextLetter || nextWord) {
    ...
    if (nextLetter) {
      ...
      if (nextWord.lastChild) {
        const lastLetterRect = nextWord.lastChild.getBoundingClientRect();
...
function newGame() {
  ...
  gameTime = parseInt(document.getElementById('timeSelect').value) * 1000;
  
  document.getElementById('timeSelect').disabled = (window.timer !== null);
...
function getWpm() {
  ...
  if (window.gameStart) {
    ...
    elapsedTimeMs = Math.min(currentTime - window.gameStart, gameTime);
  } else {
    elapsedTimeMs = gameTime;
  }
...
function gameOver() {
  ...
  document.getElementById('info').innerHTML = `WPM: ${Math.round(result)} | Acc: ${accuracy}%`;
...
document.getElementById('game').addEventListener('keyup', ev => {
  ...
  if (isBackspace) {
    const extraLetters = currentWord.querySelectorAll('.letter.incorrect.extra');
    if (extraLetters.length > 0 && !currentLetter) {
    ...
    if (currentLetter && isFirstLetter && currentWord.previousSibling) {
      ...
      const prevWordLetters = [...prevWord.children];
    ...
    if (currentLetter && !isFirstLetter) {
      ...
      if (prevLetter) {
        ...
        prevLetter.className = 'letter current';
    ...
    if (!currentLetter) {
      if (currentWord.children.length > 0) {
        ...
        if (currentWord.previousSibling) {
          ...
          if (currentWord.previousSibling.lastChild) {
...
document.getElementById('timeSelect').addEventListener('change', () => {
  if (!window.timer || document.querySelector('#game.over')) {
    gameTime = parseInt(document.getElementById('timeSelect').value) * 1000;
    document.getElementById('info').innerHTML = (gameTime / 1000) + '';
  }
});
